...
def feature_extraction(level):
    ...
    for i,filename  in enumerate(col_files):
        ...
        l = np.max(new_gray)+1  
        P_1_0 = greycomatrix(new_gray, distances=[2], angles=[radians(90)], levels=l, symmetric=False, normed=True)
...
sel_features = 2     # numero de características seleccionada
...
while(umbral<t):
    ...
    for i in range(len(combs)):
        ...
        J[i],Gn, Vk = fisher_extraction_list(sub_data,clases)
        oldJ = J
        old_comb= combs
